...
VM *VM_new(int rate, int cpuSpeed)
{	
  VM *vm ;
	int cpuClk;
  ...
  if ((int)cpuSpeed == 0)
	  cpuClk = MSX_CLK;
  else cpuClk = MSX_CLK *2;

	  
	  vm->psg = PSG_new(cpuClk, rate) ;
	  vm->scc = SCC_new(cpuClk, rate) ;
	  vm->opll = OPLL_new(cpuClk, rate) ;
	  vm->sng = SNG_new(cpuClk, rate) ;
	  vm->opl = OPL_new(cpuClk, rate) ;
