...
static k_uint8 *memfind(k_uint8 *buff, int bsize, char *str, int len)
{
  k_uint8 *p;
...
static int mbmdrv_init()
{
  ...
  drv_top = memfind(drv_code, drv_size, "AB\0", 4);
  if(!drv_top) return 1;
  drv_size -= (drv_top - drv_code);

  tmp = memfind(drv_top, drv_size, "MBPLAY", 7);
  if (!tmp) return 1;
  mbplay = read_address(tmp + 7);

  tmp = memfind(drv_top, drv_size, "MBMLOAD", 8);
  if (!tmp) return 1;
  mbmload = read_address(tmp + 8);

  tmp = memfind(drv_top, drv_size, "MBKLOAD", 8);
  if (!tmp) return 1;
